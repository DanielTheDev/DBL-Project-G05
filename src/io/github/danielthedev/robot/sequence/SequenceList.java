...
public class SequenceList {
	...
	public boolean executeSequence(SequenceType sequenceType, Robot robot) {
		...
		if (!(sequenceItem instanceof Delay))
	...
	public int findNextSequence(SequenceType type) {
		...
		for (int x = sequenceIndex; x < this.sequenceList.size(); x++) {
			if (this.sequenceList.get(x).getType() == type)
				return x;
	...
	public void registerSequences(Robot r) {
		...
		this.sequenceList
				.add(new SequenceFunction(SequenceType.STARTUP, "Testing Conveyer", r.getBeltController()::testBelt));
		...
		this.sequenceList
				.add(new SequenceFunction(SequenceType.RUNTIME, "Grabbing Disk", r.getArmController()::grabDisk));
		this.sequenceList.add(new Delay(SequenceType.RUNTIME, Delay.CONVEYER_BELT_MOVE_DELAY));
		this.sequenceList
				.add(new SequenceFunction(SequenceType.RUNTIME, "Moving item", r.getBeltController()::moveItem));
		this.sequenceList
				.add(new SequenceFunction(SequenceType.SHUTDOWN, "Stopping belt", r.getBeltController()::stop));
		this.sequenceList
				.add(new SequenceFunction(SequenceType.SHUTDOWN, "Retracting arm", r.getArmController()::retractArm));
	}
}
